...
const PostCard = ({ href, children, className, coverImage, src, userName, ...props }: CardProps) => {
  return (
    <Link href={href} className={cx(className, 'card-link')}>
      <Container margin="none" className={cx('card')} {...props}>
        ...
        <Container margin="none" className={cx('card-gallery-container')}>
          <Gallery urls={coverImage} className={cx('card-gallery', `card-${coverImage.length}`)} />
        </Container>
