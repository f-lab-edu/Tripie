...
const useAwsSearch = (data: { name: string; pointOfInterest?: string }[], context: ChatFunnelProps['context']) => {
  const res = useQuery({
...
useAwsSearch.queryKey = (chatItems: { name: string; pointOfInterest?: string }[]) => {
