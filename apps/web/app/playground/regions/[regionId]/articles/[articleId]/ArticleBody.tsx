...
const ArticleBody = ({ items }: { items: any }) => {
  return items.map((item: BodyItemProps, index: number) => {
    ...
    switch (type) {
      ...
      case 'links':
        return <ArticleLink key={index + JSON.stringify(item.value.links)} item={item} />;
      case 'embedded':
        return <ArticleEmbedded item={item} key={index + JSON.stringify(item)} />;
      case 'itinerary':
