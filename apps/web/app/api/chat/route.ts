...
export async function getTripPlan(context: TripPlannerContext) {
  ...
  const result = await generateObject({
    ...
    // const googleSearchPlaces = await Promise.all(places.map(place => fetch('/api/aws/lambda?place=' + place)));

    // const googleSearchPlaces = await Promise.all(places[0].map(place => fetch('/api/aws/lambda?place=' + place)));
    // console.log('googleSearchPlaces', googleSearchPlaces);
    ...
    //   placeSet.map(place => {
    //     const result = postAwsPlace(place, country);
    //     console.log(
    //       'result',
    //       result.then(v => v)
    //     );
