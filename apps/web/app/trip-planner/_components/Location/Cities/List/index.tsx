...
const CityList = ({ cities, selected, setSelected }: Props) => {
  ...
  const handleSelect = (city?: string) => {
    ...
    if (city == null) {
      ...
      cities.every(city => new Set(selected).has(city)) ? setSelected([]) : setSelected(cities);
      return;
    }
    // 특정 도시 선택
    if (new Set(selected).has(city)) {
